...
from agents import (
    classify_query, get_standard_node_coords, get_deep_research_node_coords,
...
async def smart_orchestrator_stream(
    ...
    try:
        ...
        if conversation_id and user_id:
            print(f"[smart_orchestrator] Loading memory context for conv={conversation_id}, user={user_id}")
            memory_context = await get_conversation_memory_context(conversation_id, user_id)
            if memory_context:
                print(f"[smart_orchestrator] Memory context loaded: {len(memory_context)} chars")
            else:
                print(f"[smart_orchestrator] No prior memory context found.")
        else:
            print("[smart_orchestrator] No conversation_id provided — fresh start.")
        ...
        if path == "standard":
            ...
            result = await run_tool_agent(task, user_id=user_id or "default_user", memory_context=memory_context)
        ...
        # ─── Code Path ───────────────────────────────────────────────────
            ...
            async for event_str in run_coding_agent_sse(task, yield_event, memory_context=memory_context):
                yield event_str
...
async def run_coding_agent_sse(
    ...
    """
    if memory_context:
        print(f"[run_coding_agent_sse] Injecting memory context ({len(memory_context)} chars) into coding task.")
